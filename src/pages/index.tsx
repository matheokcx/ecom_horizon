    ...
    <>
      <div className="w-screen h-screen flex flex-row items-center justify-center bg-gradient-to-br from-blue-800 to-black bg-fixed font-sans text-black">
        ...
        <div className="w-1/2 h-full flex flex-col items-center pt-24 gap-16 bg-white text-black rounded-xl">
          ...
          <input type="newMail" value={mailValue} onChange={(e: any) => setMailValue(e.target.value)} placeholder="Your mail" className="w-5/6 h-8 rounded-lg p-1 border-2 border-gray-400 bg-white text-black" />
          <input type="password" value={passwordValue} onChange={(e: any) => setPasswordValue(e.target.value)} placeholder="Your password" className="w-5/6 h-8 rounded-lg p-1 border-2 border-gray-400 bg-white text-black" />
          <button onClick={() => connection()} className="w-1/3 h-10 rounded-xl text-white bg-blue-700 transition-transform hover:-translate-y-2">Create</button>
          <p className="text-red-400">{errorMessage}</p>
        </div>
      </div >
    </>
